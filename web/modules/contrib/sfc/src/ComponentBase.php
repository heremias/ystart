...
class ComponentBase extends PluginBase implements ComponentInterface, ContainerFactoryPluginInterface, ComponentFormInterface, ComponentActionsInterface, ComponentFilenameInterface {
  ...
  public function getLibrary() {
    ...
    if ($this->hasCss()) {
      $library['css']['theme'][file_url_transform_relative(file_create_url("$directory/$name.css"))] = [];
    }
  ...
  protected function doWriteAssets($directory, $name) {
    ...
    if ($this->hasAttachments()) {
      ...
      if (isset($attachments['vanilla_js']) && $attachments['vanilla_js']) {
        ...
        if ($attachments['attach']) {
          $js .= "    attach: function attach(context, settings) {\n";
          $js .= "      $(" . json_encode($attachments['selector']) . ", context).addBack(" . json_encode($attachments['selector']) . ").once('sfcAttach').each(function () {\n";
          $js .= $attachments['attach'] . "\n";
          $js .= "      });\n";
          $js .= "    },\n";
        }
        $js .= "    detach: function detach(context, settings, trigger) {\n";
        if ($attachments['detach']) {
          $js .= "      $(" . json_encode($attachments['selector']) . ", context).addBack(" . json_encode($attachments['selector']) . ").once('sfcDetach').each(function () {\n";
          $js .= $attachments['detach'] . "\n";
          $js .= "      });\n";
        }
        $js .= "      var element = $(" . json_encode($attachments['selector']) . ", context).addBack(" . json_encode($attachments['selector']) . ");element.removeOnce('sfcAttach');element.removeOnce('sfcDetach');\n";
        $js .= "    },\n";
        $js .= "  }\n";
