...
class ComponentDiscovery implements ComponentDiscoveryInterface {
   ...
   */
  protected $defaults = [
    // Human readable label for components.
    'name' => '',
    // Description of the component for the admin UI.
    'description' => '',
    // The type of plugin to create with this component.
    // Current options are 'block','library', or 'plugin'.
    'type' => 'block',
    // JS & CSS files for inclusion as a library for this component.
    'js' => [],
    'css' => [],
    ...
    'dependencies' => [],
    // Define the machine name of the component that this is going to override.
    'parent' => '',
  ];
  ...
  /**
   * Generates an array of sort component data.
   *
  ...
  public function getComponents() {
    // Set up the default array of components.
    $components = [
      'block' => [],
      'library' => [],
      'plugin' => [],
    ...
    // Process each component file and create the list of components.
    foreach ($component_files as $name => $filepath) {
      ...
      if ($file_data = $this->parse($filepath)) {
        ...
        // Organize the components by type (block, library, override);.
   ...
   *
   * We want to only look in the "components" folder of any activated modules
   *  or themes.
   *
   * @return array
  ...
  protected function parse($filepath) {
    ...
    if (file_exists($filepath)) {
      ...
      if (!empty($missing_keys)) {
        unset($parsed_data);
        $this->logger->notice('Component parsing error: missing required keys (' . implode(', ', $missing_keys) . ') in ' . $filepath);
      }
    }
    return $parsed_data;
  }
